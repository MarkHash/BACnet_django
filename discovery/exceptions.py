...
class EnergyAnalyticsError(BACnetError):
    """Base exception for energy analytics operations."""


class InsufficientDataError(EnergyAnalyticsError):
    """Not enough temperature readings for analysis."""

    def __init__(self, device_id, data_count, required_count=5):
        self.device_id = device_id
        self.data_count = data_count
        self.required_count = required_count
        super().__init__(
            f"Device {self.device_id}: Only {self.data_count} readings, "
            f"need {self.required_count}+ ."
        )


class TemperatureDataError(EnergyAnalyticsError):
    """Invalid or corrupted temperature data."""

    def __init__(self, device_id, details):
        self.device_id = device_id
        self.details = details
        super().__init__(f"Device {self.device_id}: {self.details}")


class MLForecastError(EnergyAnalyticsError):
    """ML forecasting operation failed."""

    def __init__(self, device_id, original_error=None):
        self.device_id = device_id
        self.original_error = original_error
        super().__init__(
            f"Device {self.device_id}: ML forecasting failed {self.original_error}"
        )


class AnomalyDetectionError(BACnetError):
    """Base exception for anomaly detection operations."""


class InsufficientAnomalyDataError(AnomalyDetectionError):
    """Not enough historical data for anomaly detection."""

    def __init__(self, point_id: int, data_count: int, required_count: int = 5):
        self.point_id = point_id
        self.data_count = data_count
        self.required_count = required_count
        super().__init__(
            f"{self.point_id} has only {self.data_count} readings, "
            f"need {self.required_count}+ ."
        )


class StatisticalCalculationError(AnomalyDetectionError):
    """Error in statistical calculations (division by zero, etc.)."""

    def __init__(self, point_id: int, method: str, details: str = ""):
        self.point_id = point_id
        self.method = method
        self.details = details
        super().__init__(
            f"{self.point_id} has calculation error in {self.method}: {self.details}"
        )
